...
To perform the first step, the function `create_landmark_dataset` selects the patients that at the landmark time have entered the study (defined as having had their first assessment) and have not been censored or had any events. It also censors all individuals that have event times that are larger than \code{x_hor} as this is part of the landmarking approach. 
...
We can also see that event times greater than \code{x_hor} have been censored.

```{r 8}
max(data_landmark$event_time)
```
...
```{r 9}
data_landmark<-return_ids_with_LOCF(data=data_landmark,patient_id="id",covariates=c("ethnicity","smoking","diabetes","sbp_stnd","tchdl_stnd"),covariates_time=c(rep("response_time_sbp_stnd",4),"response_time_tchdl_stnd"),x_L=60)
length(unique(data_landmark$id))
```
...
We can see that fitting this model returns the warning `Loglik converged before variable  2 ; coefficient may be infinite.`. Taking a look at the model fit contained in `data_model_landmark_LOCF$model_survival`, the upper confidence interval for the coefficient for atrial_fibrillation is `inf` for cause 2 for fold number 8. This is due there being few individuals that have ethnicity 'Indian' and few individuals that experienced death from other causes. However, as only the point estimate of the coefficient is needed to calculate the risk prediction, not having the standard error is not a problem. Indeed we have obtained the risk predictions and can take a look at their distribution.


```{r 12}
plot(density(100*data_model_landmark_LOCF$data$event_prediction), xlab="Predicted risk of CVD event (%)",main="")
```
