...
Below is a step-by-step guide to use this package to use a landmark model to predict the 5-year risk of cardiovascular disease (CVD). In particular, this example produces the landmark model for females aged 60 years old. To produce the full landmark model to allow predictions for any age, a series of these landmark models should be fit for at ages \dots 57,58,59,61, 62 \dots.
...
* `data_repeats` contains repeat measurement data from CVD risk assessments. Each row corresponds to a different assessment and an individual may have multiple assessments. The dataset `data_repeat` contains information about an individual's ethnicity, smoking status, diabetes, deprivation score, diagnosis of atrial fibrillation, and standardised systolic blood pressure (SBP). These were recorded on the date specified in `response_date_sbp_stnd`. The standardised total cholesterol to high density lipoprotein (HDL) ratio is also recorded as part of the assessment but was recorded on the date specified in `response_date_tchdl_stnd`, this is up to a month after the other variables were measured.
...
```
We are now ready to perform the landmarking analysis. Firstly, we fit the landmark model using the LOCF longitudinal submodel with the cause-specific survival submodel. For more information, see the Details section of the \code{fit_LOCF_landmark_model} documentation.
...
Secondly, we fit the landmark model using the LME longitudinal submodel with the cause-specific survival submodel.  The variables standardised systolic blood pressure (SBP) and standardised total cholesterol to high density lipoprotein (HDL) ratio are repeatedly measured and so these can be included as random effects in the LME model.   
