...
for the LOCF approach, the LOCF values at the landmark age `x_L` are extracted for `covariates` and so at least one observation is needed and so at least one observation is needed of each of these.
This condition is necessary for the LME approach, as the LOCF values at the landmark age `x_L` are extracted for `fixed_effects` and so at least one observation is needed of each of these.
The function `fit_LME_landmark` allows for no observations in `random_effects` by the landmark age because the LME model can handle missing data in the random effects.
The individuals in that satisfy this condition are selected using the function `return_ids_with_LOCF`.

* exited the study after and not including time `x_L`.

After this dataset is formed for each of the landmark ages, the second step involves adding the cross-validation fold
to the individuals in the dataset. The datasets that were just formed are re-combined in
order to ensure that the cross-validation fold assignment is consistent across
each of the datasets. The cross-validation folds
may be defined in two ways:

the user sets parameter `k` which randomly assigns cross-validation folds to each individual using the function
`add_cv_number`

the user sets parameter `cross_validation_df` which refers to a data frame that contains the user-defined assignment of
